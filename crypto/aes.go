...
func DecryptString(ciphertext, key string) (string, error) {
	...
	if err != nil {
		log.
			Error().Stack().
			Err(err).
			Msgf("failed to decode ciphertext: %s", err.Error())
		return "", err
	}
	...
	if err != nil {
		log.
			Error().Stack().
			Err(err).
			Msgf("failed to generate cipher key block: %s", err.Error())
		return "", err
	}
	...
	if err != nil {
		log.
			Error().Stack().
			Err(err).
			Msgf("failed to wrap block cipher in GCM: %s", err.Error())
		return "", err
	}
	...
	if err != nil {
		log.
			Error().Stack().
			Err(err).
			Msgf("failed to decrypt ciphertext: %s", err.Error())
		return "", err
	}
...
func EncryptString(plaintext, key string) (string, error) {
	...
	if key == "" {
		...
		if _, err := rand.Read(cipherKey); err != nil {
			log.
				Error().Stack().
				Err(err).
				Msgf("failed to generate random key: %s", err.Error())
			return "", err
		}
	}
	...
	if _, err = io.ReadFull(rand.Reader, nonce); err != nil {
		log.
			Error().Stack().
			Err(err).
			Msgf("failed to generate nonce: %s", err.Error())
		return "", err
	}
